...
const getRedirectionRules = async () => {
	if (!GQL_ENDPOINT || !host) {
		console.warn('[next.config.js] Missing GQL_ENDPOINT or host env vars — skipping redirects.');
		return [];
	}
	...
	try {
		...
		if (!data.publication) {
			console.warn('[next.config.js] No publication found — skipping redirects.');
			return [];
		}
		...
		console.warn('[next.config.js] Failed to fetch redirection rules:', error.message || error);
		return [];
	}
};
